...
describe('Comptroller', () => {
  ...
  describe('liquidity', () => {
    ...
    it("allows entering 3 markets, supplying to 2 and borrowing up to collateralFactor in the 3rd", async () => {
      ...
      ({1: liquidity, 2: shortfall} = await call(vToken3.comptroller, 'getHypotheticalAccountLiquidity', [user, vToken3._address, Math.floor(c2), 0]));
      expect(liquidity).toEqualNumber(collateral);
      expect(shortfall).toEqualNumber(0);

      ({1: liquidity, 2: shortfall} = await call(vToken3.comptroller, 'getHypotheticalAccountLiquidity', [user, vToken3._address, 0, Math.floor(c2)]));
      expect(liquidity).toEqualNumber(c1);
      expect(shortfall).toEqualNumber(0);

      ({1: liquidity, 2: shortfall} = await call(vToken3.comptroller, 'getHypotheticalAccountLiquidity', [user, vToken3._address, 0, collateral + c1]));
      expect(liquidity).toEqualNumber(0);
      expect(shortfall).toEqualNumber(c1);

      ({1: liquidity, 2: shortfall} = await call(vToken1.comptroller, 'getHypotheticalAccountLiquidity', [user, vToken1._address, amount1, 0]));
  ...
  describe("getHypotheticalAccountLiquidity", () => {
    ...
    it("returns collateral factor times dollar amount of tokens minted in a single market", async () => {
      ...
      const {0: error, 1: liquidity, 2: shortfall} = await call(vToken.comptroller, 'getHypotheticalAccountLiquidity', [from, vToken._address, 0, 0]);
      expect(error).toEqualNumber(0);
